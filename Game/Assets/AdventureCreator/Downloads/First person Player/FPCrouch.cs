...
namespace AC
{
	...
	public class FPCrouch : MonoBehaviour
	{
		...
		{
			Collider[] overlapColliders = Physics.OverlapCapsule (player.transform.position + (Vector3.up * radius), player.transform.position + (Vector3.up * (standingHeight - radius)), radius, standLayerMask);
